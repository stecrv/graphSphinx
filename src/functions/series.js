...
const sliceTimeSeries = (
  ...
  let result = [];
  if (exactDateMatch) {
    const startIndex = _.findIndex(timeSeries, (el) => el.x === startingDate);
    const endIndex = _.findLastIndex(timeSeries, (el) => el.x === endingDate);
    result =      startIndex > -1 && endIndex > -1
        ? timeSeries.slice(startIndex, endIndex)
        : [];
  } else {
    result = timeSeries.filter(
      (el) => isBeforeDate(startingDate, el.x) && isBeforeDate(el.x, endingDate)
    );
  }
  return result;
};
